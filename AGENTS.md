...
`make all` is the complete top-level build path.  It may synchronize the pinned
build dependency, then must build all three generated executable artifacts, run
the TAP semantic suite against those generated bytes, and leave all three
corresponding SHA-256 files in `dist/`.  `make build` remains the canonical
offline artifact-generation step and must not acquire dependencies implicitly.
Generated provenance is owned by the build, remains comments only, and must be
kept outside body transformations so comment stripping or minification cannot
erase provenance or introduce executable AWK state.
