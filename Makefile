...
.PHONY: adr-index all build check checksums clean deps-docs deps-docs-check distclean docs docs-canary docs-clean integration-clean test test-source test-dist test-doxygen verify-bashdeps FORCE
...
## Run one TAP stream covering maintained source and consumer output.
test: build
...
## Build and run the TAP semantic suite against the exact consumer artifact.
test-dist: build
	...
	AWK_BIN="$(AWK_BIN)" sh ./tests/run-tests.sh "dist=$(DIST_FILTER)"
